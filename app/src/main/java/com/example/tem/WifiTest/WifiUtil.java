...
// 定义几种加密方式，一种是WEP，一种是WPA，还有没有密码的情况
    ...
    public boolean OpenWifi() {
        boolean bRet = true;
        Log.e("111111111111","1"+mWifiList);
        Log.e("111111111111","wifi是否开启 "+mWifiManager.isWifiEnabled());

        if (!mWifiManager.isWifiEnabled()) {//判断是否开启
            //注意，调用setWifiEnabled后，系统进行wifi模块的开启需要一定时间，此时可通过wifiManager.getWifiState()获取的状态来判断是否完成。
            bRet = mWifiManager.setWifiEnabled(true);
            Log.e("111111111111","1"+bRet);
            ...
            bRet = mWifiManager.setWifiEnabled(true);
            Log.e("111111111111","2"+bRet);
            for(int i=0;i<mWifiList.size();i++){
                Log.e("111111111111","wifi"+i+":"+mWifiList.get(i));
            }
        }
        Log.e("111111111111","2bRet"+bRet);
        Log.e("111111111111","2"+mWifiList);
        return bRet;
    }
    ...
    private WifiConfiguration IsExsits(String SSID) {
        ...
        Log.e("111111111111","以前配置过de网络:"+existingConfigs);
    ...
    public WifiConfiguration createWifiInfo(String SSID, String Password, int Type){
        ...
        Log.e("111111111111", "SSID = " + SSID + "## Password = " + Password + "## Type = " + Type);
        WifiConfiguration config = new WifiConfiguration();
        config.allowedAuthAlgorithms.clear();
        config.allowedGroupCiphers.clear();
        config.allowedKeyManagement.clear();
...
*/
    ...
    public boolean forgetWifi(String SSID){
        WifiConfiguration tempConfig = this.IsExsits(SSID);
        Log.e("111111111111","tempConfig="+tempConfig);

        if (tempConfig != null) {
            Log.e("111111111111","mWifiManager.removeNetwork(tempConfig.networkId)="+mWifiManager.removeNetwork(tempConfig.networkId));
...
/*先执行删除wifi操作，1.如果删除的成功说明这个wifi配置是由本APP配置出来的；
        ...
        } else
        {
            ...
            {
                Log.e("111111111111","这个wifi是连接过的");
                //这个wifi是连接过的，如果这个wifi在连接之后改了密码，那就只能手动去删除了
                netId = getExitsWifiConfig(SSID).networkId;
            } else
            {
                //没连接过的，新建一个wifi配置
                Log.e("111111111111","新建一个wifi配置");
                netId = mWifiManager.addNetwork(createWifiInfo(SSID, password, Type));//新创建的网络描述的ID。这在其他操作中用于指定要执行的网络。失败时返回-1。
                Log.e("111111111111","netId:"+netId);
            }
        }
